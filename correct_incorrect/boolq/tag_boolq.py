...
- Correct answers get <C_READ>
- Incorrect answers get <U_READ>
...
def process_boolq_model(model_name, base_dir="correct_incorrect/boolq"):
    """Process correct and incorrect files for a model."""
    
    model_dir = os.path.join(base_dir, model_name)
    
    # Define file paths
    correct_file = os.path.join(model_dir, f"{model_name}_boolq_train_correct.json")
    incorrect_file = os.path.join(model_dir, f"{model_name}_boolq_train_incorrect.json")
    
    correct_tagged_file = os.path.join(model_dir, f"{model_name}_boolq_train_correct_tagged.json")
    incorrect_tagged_file = os.path.join(model_dir, f"{model_name}_boolq_train_incorrect_tagged.json")
    combined_tagged_file = os.path.join(model_dir, f"{model_name}_boolq_train_tagged.json")
    
    print(f"\n{'='*70}")
    print(f"Processing BoolQ predictions for: {model_name}")
    print(f"{'='*70}")
    ...
    # Tag correct predictions with <C_READ>
    print(f"\nTagging correct predictions with <C_READ>...")
    correct_tagged = tag_predictions(correct_predictions, "<C_READ>")
    
    # Tag incorrect predictions with <U_READ>
    print(f"Tagging incorrect predictions with <U_READ>...")
    ...
    print(f"{'='*70}")
    print(f"Correct predictions: {len(correct_tagged)} (<C_READ>)")
    print(f"Incorrect predictions: {len(incorrect_tagged)} (<U_READ>)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Tag BoolQ predictions with confidence tokens"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        "--base-dir",
        type=str,
        default="correct_incorrect/boolq",
        help="Base directory for BoolQ predictions"
    )
    ...
    for model in models:
        try:
            process_boolq_model(model, args.base_dir)
